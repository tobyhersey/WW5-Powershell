$file = "C:\users\devadmin\Desktop\starwars.csv"

##Make objects out of the Data
function WW5-add-user ($file) {

##Import the file
$import = Import-Csv  $file



$import | % { 
  $roleID = $($_.Role)
  $Fullname = $($_.Fullname)
  $email = $($_.Email)
  $Username = $($_.Username)
  $Password = $($_.Password)


$newuser = @"
{
"roleId":"$roleID",
"fullName":"$fullname",
"email":"$email",
"userName":"$username",
"password":"$password"
}
"@


$token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ftyGEDp0y5pK7z3iQ9twkwZko7kX958rJIJmGwbd_Yw"



try {
        $result = invoke-webrequest -Method POST  -uri "http://10.10.10.109/api/users" -Body $newuser -Headers @{Authorization = $token; Accept = 'application/json'} -ContentType application/json
}
catch {
        $result = $_.Exception.Response.GetResponseStream()
        $reader = New-Object System.IO.StreamReader($result)
        $responseBody = $reader.ReadToEnd();
}

}
}


